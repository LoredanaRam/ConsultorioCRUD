...
{
    rebuildAppointmentTable = () =>
    {
        this.deleteAppointmentTable();
        this.createUpdatedTable();
    };
    
    createUpdatedTable = async () => {
        let tableRequest = await fetch(tableURL);
        let tableObject = await tableRequest.json();
        let tableHTML = "";
        tableObject.forEach(appointment => {
          tableHTML += `
          <tbody id="${appointment.id}" class="table-card-appointment">
            <tr class="table-appointments-row bg-dark-lighter">
              <td scope="row">${appointment.name}</td>
              <td>${appointment.topic}</td>
              <td>${appointment.date}</td>
              <td class="d-none d-sm-table-cell">${appointment.description}</td>
              <td>
                <input type="checkbox" id="isDone${appointment.id}" name="isDone" value="isDone">
              </td>
              <td>
    ...
    deleteAppointmentTable = () => document.getElementById('appointments-table').innerHTML = `
